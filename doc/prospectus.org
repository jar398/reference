...
When we calculate mutual information there are several kinds of
variation over which we can sample, depending on the region.
  1. All or most of the variables change at once; this could be due to
     an object coming into the field of the agent's sensors, or leaving.
...
[how does the agent 'know' which case applies?]
