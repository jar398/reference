...
Sensors read the state the world; they obtain information from the
agent's environment.  The agent can detect which variables (thus read)
go which other ones, and we can suppose that they form 'object
hypotheses' consisting of variables that they know about that go together.
...
the same world has physically dictated peripheral sensations arising
from some 'real' object to both agents.
...
we would have to measure payoffs in all possible world states, while
...
limited data, by reasoning about the world (using our own knowledge of
it) and applying common sense assumptions to seek the best hypotheses
...
spell out the implications of the framework for 'knowledge
