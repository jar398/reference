#+TITLE: Demystifying compositional semantics - a prospectus
...
not be difficult.  The kind of help I'm looking for has more to do
with overall organization and gaps.
...
The main presentation is in four parts.  The first three parts set the
stage for addressing the question of compositionality, and the fourth
does so.  Part 1 presents an 'ontology', which can be taken to be
either the description of a contrived world, or an abstraction in a
certain direction of the real world.  Part 2 talks about agents, which
are entities that 'act' on the world and interact with one another.
Part 3 is about communication between agents.
...
A variable is a function from world states to values; sensitive
perhaps not only to one particular state, but perhaps also to some or
all previous or future states.  (Instead of present, past, and future
time we could consider any other independent variable.  Since this
doesn't matter very much I'll take the independent variable to be
time.)  The value of a variable could be a truth value (true/false),
number, letter, or anything similarly simple and abstract; a 'scalar'
if you will.
...
Similar ideas are for variables to be dependent or correlated, or to
have mutual information
[[https://en.wikipedia.org/wiki/Mutual_information]], the latter because
knowing some of the variables give you information about the others
(or at least about their distribution, in a probabilistic setting).
...
Horwich, Kripke, Harman, Gopnik, Yablo, many others.
